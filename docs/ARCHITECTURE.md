...
- Still open: `npm audit` (2 critical / 16 high, mostly `next` and `tar`); a CSP; the `cv-data` placeholder singleton and `sample_mflix` in Atlas (user-side deletes); rotating `NASA_KEY`.
